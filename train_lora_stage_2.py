...
def build_argparser():
    ...
    p.add_argument("--transformer_weights_dir", type=str, default=None,
                   help="Dir with diffusion_pytorch_model-*.safetensors")
    p.add_argument("--transformer_weights", type=str, default=None,
    ...
    p.add_argument("--adapter_prefix", type=str, default="diffusion_model.",
...
def _load_video(path: str, side: int, target_T: int) -> torch.Tensor:
    ...
    # resize per-frame
    vframes = torch.stack([resize(f.permute(2,0,1), [side, side], antialias=True).permute(1,2,0)
                           for f in vframes], dim=0)
...
class MediaDataset(Dataset):
    def __init__(self, root: str, use_480p: bool, use_720p: bool,
                 frames_list: List[int], max_samples: Optional[int]=None, shuffle: bool=False):
        root = Path(root)
        ...
        def scan_img(reso: int):
            ...
            for img in glob.glob(str(d / "*")):
                if os.path.splitext(img)[1].lower() not in (".png", ".jpg", ".jpeg", ".bmp", ".webp"):
                    continue
        ...
        if use_480p: scan_img(480); scan_vid(480)
        if use_720p: scan_img(720); scan_vid(720)
...
def pad_stack(tokens: List[torch.Tensor], max_len: int) -> Tuple[torch.Tensor, torch.Tensor]:
    b = len(tokens)
    d = tokens[0].shape[-1]
...
def _load_filtered_state(weights_dir: Optional[str], weights_file: Optional[str]) -> Dict[str, torch.Tensor]:
    from safetensors.torch import load_file, safe_open
    if weights_dir:
        # merge shards
        state = {}
        for f in sorted(glob.glob(os.path.join(weights_dir, "diffusion_pytorch_model-*.safetensors"))):
...
def call_dit(model: nn.Module, x_t: torch.Tensor, t: torch.Tensor,
             context: torch.Tensor, mask: Optional[torch.Tensor]) -> torch.Tensor:
    # Common signatures:
...
def make_velocity_training_pair(x1: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
...
def main(args):
    ...
    if args.resume_adapter and os.path.exists(args.resume_adapter):
        load_peft_adapter(model, args.resume_adapter, prefix=args.adapter_prefix, alpha=args.alpha)
    ...
    print(f"[lora] Trainable LoRA params: {n_params/1e6:.3f} M across targets={targets} (freeze_kv={args.freeze_kv})")
    ...
    ds = MediaDataset(args.data_root, args.use_480p, args.use_720p,
                      frames_list=frames_list, max_samples=args.max_samples, shuffle=args.shuffle)
    if len(ds) == 0: raise RuntimeError("No samples found under the given config.")
    ...
    loader = DataLoader(ds, batch_size=args.batch_size, shuffle=True,
                        num_workers=args.num_workers, pin_memory=True, collate_fn=_collate, drop_last=True)

    # Optimizer
    opt = torch.optim.AdamW(params, lr=args.lr, weight_decay=args.weight_decay, betas=(0.9, 0.95))
    scaler = torch.cuda.amp.GradScaler(enabled=False)  # bf16 uses autocast without scaler
    model.train()
    ...
    while step < args.steps:
        for batch in loader:
            ...
            if step % args.save_every == 0 or step == args.steps:
                ...
                export_peft_adapter(model, str(out_dir / "adapter_model.safetensors"), prefix=args.adapter_prefix)
