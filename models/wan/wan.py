...
class WanPipeline(BasePipeline):
    ...
    def __init__(self, config):
        ...
        self.train_text_encoder = self.model_config.get('train_text_encoder', False) # (bool, default false)
        self.text_train_blocks = self.model_config.get('text_train_blocks', 0) # (int = how many top blocks to train; default 0 = all frozen)
        ...
        self.train_text_encoder = (not self.cache_text_embeddings) and self.model_config.get('train_text_encoder', False)
        self.text_train_blocks = int(self.model_config.get('text_train_blocks', 0))

        # default: eval if not training
        ...
        if self.train_text_encoder:
            ...
            # Optionally train only the top N blocks (e.g., last 2–4)
            try:
                blocks = list(self.text_encoder.model.blocks)
                total = len(blocks)
                start = max(0, total - self.text_train_blocks) if self.text_train_blocks > 0 else total
                for i, blk in enumerate(blocks):
                    req = (i >= start)
                    for p in blk.parameters():
                        p.requires_grad_(req)
            except Exception:
                # Fallback: if structure differs, train whole encoder
    ...
    def load_diffusion_model(self):
        ...
        if self.model_config.get('freeze_transformer', True):
    ...
    def save_text_encoder_full(self, save_dir):
        dtype_to_save = self.model_config.get('save_dtype', torch.bfloat16)
        enc_sd = {k: v.to(dtype_to_save) if v.is_floating_point()
                  else v
                  for k, v in self.text_encoder.model.state_dict().items()}
        safetensors.torch.save_file(enc_sd, save_dir / 'text_encoder.safetensors',
                                    metadata={'format': 'pt'})

    def save_adapter(self, save_dir, peft_state_dict):
        # existing LoRA save (Comfy format)
    ...
    def prepare_inputs(self, inputs, timestep_quantile=None):
        ...
        if self.cache_text_embeddings:
            ...
            # Optional: caption augmentation (trim to keywords some % of the time)
            caption_list = inputs['caption']
            aug_cfg = self.model_config.get('caption_aug', {})
            if aug_cfg.get('enable', False):
                # replace the body inside: if aug_cfg.get('enable', False):
                trim_prob = float(aug_cfg.get('trim_prob', 0.0))
                cis = bool(aug_cfg.get('case_insensitive', True))
                kws = list(aug_cfg.get('keywords', []))
                single_token = bool(aug_cfg.get('single_token', True))
                flags = re.IGNORECASE if cis else 0
                pat = re.compile(r'\b(' + '|'.join(map(re.escape, kws)) + r')\b', flags) if kws else None
                trimmed = []
                for s in caption_list:
                    if pat and random.random() < trim_prob:
                        tokens = pat.findall(s)
                        if tokens:
                            if single_token:
                                s = random.choice(tokens)  # <--- single token
                            else:
                                s = ', '.join(dict.fromkeys([t.strip() for t in tokens]))
                    trimmed.append(s)
                caption_list = trimmed
            text_embeddings_or_ids, seq_lens_or_text_mask = self.text_encoder.tokenizer(
                caption_list, return_mask=True, add_special_tokens=True
            )
    ...
    def configure_adapter(self, adapter_config):
        """
        Call the base implementation to attach LoRA everywhere we normally would,
        then selectively enable only the layers we want to *train* (and optionally
        hard-disable the others so they have no forward effect).
        """
        super().configure_adapter(adapter_config)
        self._apply_lora_filter(adapter_config)

    def _apply_lora_filter(self, adapter_config):
        """
        Filter LoRA trainability/forward impact by parameter name.
        Supports:
          - adapter.train_blocks_range = [start, end]  (inclusive)
          - adapter.target_projs      = ["k","v","q","o"]   (defaults empty = no extra filter)
          - adapter.drop_self_attn    = true/false          (default: true)
          - adapter.drop_cross_o      = true/false          (default: true)
          - adapter.filter.include    = [regex, ...]
          - adapter.filter.exclude    = [regex, ...]
          - adapter.hard_disable      = true/false          (zero out dropped LoRA params; default: true)
        """
        filt = adapter_config.get('filter', {})
        include_patterns = [re.compile(p) for p in filt.get('include', [])]
        exclude_patterns = [re.compile(p) for p in filt.get('exclude', [])]

        blocks_range = adapter_config.get('train_blocks_range', None)  # e.g. [10, 21]
        targets = set(adapter_config.get('target_projs', []))          # e.g. ["k","v"]
        drop_self_attn = bool(adapter_config.get('drop_self_attn', True))
        drop_cross_o   = bool(adapter_config.get('drop_cross_o', True))
        hard_disable   = bool(adapter_config.get('hard_disable', True))

        def want(full_name: str) -> bool:
            # Normalize LoRA param name to its base weight path
            # "...cross_attn.k.lora_A.weight" -> "...cross_attn.k.weight"
            base = re.sub(r'\.lora_[AB]\.weight$', '.weight', full_name)

            # 1) global functional drops
            if drop_self_attn and 'self_attn.' in base:
                return False
            if drop_cross_o and 'cross_attn.o.' in base:
                return False

            # 2) restrict to cross_attn.* by default (keeps FFN untouched)
            if 'cross_attn.' not in base:
                return False

            # 3) restrict projections if provided
            if targets:
                m = re.search(r'cross_attn\.(\w)\.', base)  # k/v/q/o
                if not m or m.group(1) not in targets:
                    return False

            # 4) block range
            if blocks_range:
                m = re.search(r'blocks\.(\d+)\.', base)
                if m:
                    bi = int(m.group(1))
                    if not (blocks_range[0] <= bi <= blocks_range[1]):
                        return False

            # 5) regex include / exclude
            if include_patterns and not any(p.search(base) for p in include_patterns):
                return False
            if any(p.search(base) for p in exclude_patterns):
                return False
        ...
        for n, p in self.transformer.named_parameters():
            ...
            if want(n):
                ...
                p.requires_grad_(False)
                disabled.append(n)
                # Optionally remove any forward influence from dropped adapters
                if hard_disable and p.is_floating_point():
                    p.data.zero_()

        log.info(f"[WanPipeline/LoRA filter] enabled={len(enabled)} disabled={len(disabled)}")
        # Print a few examples to make debugging easy
        for n in enabled[:6]:
            log.info(f"  ENABLED: {n}")
        for n in disabled[:3]:
            log.info(f"  disabled: {n}")
