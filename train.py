...
if __name__ == '__main__':
    ...
    if adapter_config := config.get('adapter', None):
        ...
        if hasattr(model, "text_encoder") and not model.cache_text_embeddings:
            n_txt = sum(p.numel() for p in model.text_encoder.model.parameters() if p.requires_grad)
            if is_main_process():
    ...
    def get_optimizer(model_parameters):
        ...
        if optim_config.get('gradient_release', False):
            ...
            lr_dit = kwargs.pop('lr_dit', None)  # optional; falls back to lr_base if not provided
            ...
            dit_set = {p for p in model.transformer.parameters() if p.requires_grad}
            te_set = set()
            if hasattr(model, 'text_encoder') and hasattr(model.text_encoder, 'model'):
                te_set = {p for p in model.text_encoder.model.parameters() if p.requires_grad}
    ...
    while True:
        ...
        if finished_epoch:
            ...
            if 'caption_aug' in model.model_config:
                sched = model.model_config['caption_aug'].get('trim_prob_schedule', None)
