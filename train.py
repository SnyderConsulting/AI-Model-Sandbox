...
def _count_all_layer_params(self):
    ...
    for idx, layer in enumerate(self._layer_specs):
        if isinstance(layer, ds_pipe_module.LayerSpec):
            layer_module = layer.build()
            param_counts[idx] = sum(p.numel() for p in layer_module.parameters())
...
if __name__ == "__main__":
    ...
    if adapter_config := config.get("adapter", None):
        ...
        if hasattr(model, "text_encoder") and not model.cache_text_embeddings:
            ...
            if is_main_process():
                print(f"Trainable text-encoder params: {n_txt/1e6:.2f}M")
    ...
    def get_optimizer(model_parameters):
        ...
        if optim_config.get("gradient_release", False):
            ...
            # --- Split LR support (DiT/LoRA vs Text Encoder) ---
            # Pull split LRs out of the optimizer config so they don't get passed to the optimizer ctor.
            lr_base = kwargs.pop("lr", 5e-5)
            lr_te = kwargs.pop("lr_te", None)  # e.g., 8e-6
            lr_dit = kwargs.pop(
                "lr_dit", None
            )  # optional; falls back to lr_base if not provided

            # Start from whatever grouping the pipeline already provides (preserves weight_decay/no_decay splits).
            base_groups = model.get_param_groups(model_parameters)

            # Build membership sets so we can split groups without string matching names
            te_set = set()
            if hasattr(model, "text_encoder") and hasattr(model.text_encoder, "model"):
                te_set = {
                    p for p in model.text_encoder.model.parameters() if p.requires_grad
                }

            new_groups = []
            for pg in base_groups:
                params = list(pg["params"])
                # split into TE vs non-TE; "non-TE" covers DiT/LoRA here
                te_params = [p for p in params if p in te_set]
                non_te = [p for p in params if p not in te_set]

                # clone common options from the original group (preserves weight_decay, etc.)
                common = {k: v for k, v in pg.items() if k != "params"}

                if te_params:
                    g = dict(common)
                    g["params"] = te_params
                    g["lr"] = lr_te if lr_te is not None else lr_base
                    new_groups.append(g)

                if non_te:
                    g = dict(common)
                    g["params"] = non_te
                    g["lr"] = lr_dit if lr_dit is not None else lr_base
                    new_groups.append(g)

            for k in ("lr_te", "lr_dit", "lr_lora"):
                kwargs.pop(k, None)
    ...
    while True:
        ...
        if is_main_process() and step % config["logging_steps"] == 0:
            ...
            if optimizer.__class__.__name__ == "Prodigy":
                ...
                tb_writer.add_scalar("train/prodigy_d", prodigy_d, step)
            if optimizer.__class__.__name__ in ("Automagic", "GenericOptim"):
                lrs, avg_lr = _get_automagic_lrs(optimizer)
                if avg_lr > 0:
                    tb_writer.add_histogram("train/automagic_lrs", lrs, step)
                    tb_writer.add_scalar("train/automagic_avg_lr", avg_lr, step)
        ...
        if finished_epoch:
            ...
            num_steps = 0
            epoch = new_epoch
            if "caption_aug" in model.model_config:
                sched = model.model_config["caption_aug"].get(
                    "trim_prob_schedule", None
                )
                if isinstance(sched, list) and len(sched) > 0:
                    # sched is list of [epoch_threshold, new_trim_prob] pairs
                    for th, p in sched:
                        if epoch >= th:
                            model.model_config["caption_aug"]["trim_prob"] = p
            if epoch is None:
                break
