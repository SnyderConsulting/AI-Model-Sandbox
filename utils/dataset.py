...
class SizeBucketDataset:
    ...
    def cache_latents(self, map_fn, regenerate_cache=False, trust_cache=False, caching_batch_size=1):
        ...
        if map_fn is None:
            ...
            if not shard_paths:
                raise RuntimeError(f'No cached latents found in {self.cache_dir}. '
            ...
            if iteration_order_cache_dir.exists():
                ...
                image_spec_to_latents_idx = {
                    tuple(spec): i for i, spec in enumerate(self.latent_dataset["image_spec"])
                }
                order = []
        ...
        if regenerate_cache or not iteration_order_cache_dir.exists() or not trust_cache:
            ...
            def _gen():
                for image_spec, latents_idx, caption, caption_number in order:
                    yield {'image_spec': image_spec, 'latents_idx': latents_idx,
                           'caption': caption, 'caption_number': caption_number}
            iteration_order = HFDataset.from_generator(_gen, keep_in_memory=True)
            iteration_order.save_to_disk(str(iteration_order_cache_dir))
...
class ARBucketDataset:
    ...
    def cache_latents(self, map_fn, regenerate_cache=False, trust_cache=False, caching_batch_size=1):
        ...
        for res in self.resolutions:
            area = res**2
            w = round_to_nearest_multiple(math.sqrt(area * self.ar_frames[0]), IMAGE_SIZE_ROUND_TO_MULTIPLE)
            h = round_to_nearest_multiple(area / (w if w != 0 else 1), IMAGE_SIZE_ROUND_TO_MULTIPLE)
...
class DirectoryDataset:
    ...
    def cache_latents(self, map_fn, regenerate_cache=False, trust_cache=False, caching_batch_size=1):
        ...
        for ds in datasets_list:
            ds.cache_latents(map_fn, regenerate_cache=regenerate_cache, trust_cache=trust_cache,
                             caching_batch_size=caching_batch_size)
