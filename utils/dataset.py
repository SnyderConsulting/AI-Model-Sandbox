...
def shuffle_with_seed(seq, seed=None):
...
def _cache_text_embeddings(
    ...
    def flatten_captions(example):
        ...
        for image_spec, captions, is_video in zip(
            example["image_spec"], example["caption"], example["is_video"]
        ):
            for caption in captions:
...
class SizeBucketDataset:
    ...
    def cache_latents(
        ...
        print(f"cache_latents (size bucket): {self.size_bucket}")

        iteration_order_cache_dir = self.cache_dir / "iteration_order"

        # LOAD-ONLY PATH: don't recompute latents, just attach existing shards + iteration order
        if map_fn is None:
            from datasets import Dataset, concatenate_datasets, load_from_disk

            shard_paths = sorted(self.cache_dir.glob("latents_*.arrow"))
            if not shard_paths:
                raise RuntimeError(
                    f"No cached latents found in {self.cache_dir}. "
                    f"Run once without the load-only guard to build them."
                )

            ds_list = [Dataset.from_file(str(p)) for p in shard_paths]
            self.latent_dataset = concatenate_datasets(ds_list)
            self.latent_dataset.set_format(type="torch")

            if iteration_order_cache_dir.exists():
                self.iteration_order = load_from_disk(str(iteration_order_cache_dir))
            else:
                # Rebuild iteration order from cached latents + metadata (same logic as normal path)
                image_spec_to_latents_idx = {
                    tuple(spec): i
                    for i, spec in enumerate(self.latent_dataset["image_spec"])
                }
                order = []
                for example in self.metadata_dataset.select_columns(
                    ...
                    for i, caption in enumerate(captions):
                        order.append((image_spec, latents_idx, caption, i))

                from datasets import Dataset as HFDataset

                def _gen():
                    # shuffle with deterministic seed for stability
                    import random

                    random.Random(42).shuffle(order)
                    for image_spec, latents_idx, caption, caption_number in order:
                        yield {
                ...
                iteration_order = HFDataset.from_generator(_gen, keep_in_memory=True)
                iteration_order.save_to_disk(str(iteration_order_cache_dir))
                self.iteration_order = iteration_order
            return

        # NORMAL PATH (computes if cache missing; otherwise loads)
...
class ARBucketDataset:
    ...
    def cache_latents(
        ...
        print(f"cache_latents (AR bucket): {self.ar_frames}")
        # always create size-buckets so the directory exposes datasets downstream
        for res in self.resolutions:
            area = res**2
            w = round_to_nearest_multiple(
                math.sqrt(area * self.ar_frames[0]), IMAGE_SIZE_ROUND_TO_MULTIPLE
            )
            h = round_to_nearest_multiple(
                area / (w if w != 0 else 1), IMAGE_SIZE_ROUND_TO_MULTIPLE
            )
            ...
            metadata_with_size_bucket = self.metadata_dataset.map(
                ...
                cache_file_name=str(
                    self.cache_dir / "metadata/metadata_with_size_bucket.arrow"
                ),
        ...
        # Then attach latents/iteration_order (compute if needed, else load-only if map_fn is None)
...
class DirectoryDataset:
                ...
                for image_spec in process_file(file):
                    ...
                    if self.control_path:
                        if image_file.stem not in control_file_stems:
                            raise RuntimeError(
                                f"No control file exists for image {image_file}"
                            )
    ...
    def cache_latents(
        ...
        print(f"cache_latents (dir): {self.path}")
        datasets_list = (
            self.size_bucket_datasets
            if self.use_size_buckets
...
class Dataset:
    ...
    def post_init(
        ...
        self.buckets = []
        for ds_list in datasets_by_size_bucket.values():
            self.buckets.append(ConcatenatedBatchedDataset(ds_list))
