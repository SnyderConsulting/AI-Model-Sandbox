...
class Saver:
    def __init__(self, args, config, is_adapter, save_root, model, train_dataloader, model_engine, pipeline_model):
        self.args = args
        self.config = config
        # Allow either a root-level or [model] flag
        self.save_text_encoder_full = bool(
            config.get('save_text_encoder_full', False) or (isinstance(config.get('model', {}), dict) and config['model'].get('save_text_encoder_full', False))
        )
        self.save_diffusion_model = bool(
            config.get('save_diffusion_model', True) or
            (isinstance(config.get('model', {}), dict) and
             config['model'].get('save_diffusion_model', True))
        )
    ...
    def save_adapter(self, name):
        ...
        if dp_id == 0 and stage_id == 0:
            ...
            # Optionally also write a full text-encoder artifact (~11GB bf16)
            if self.save_text_encoder_full and hasattr(self.model, 'save_text_encoder_full'):
                try:
                    self.model.save_text_encoder_full(save_dir)
                except Exception as e:
                    logger.warning(f"save_text_encoder_full failed: {e}")
